...
namespace Fiche_Reparation.Controllers
{
    public class DemandeReparationController : Controller
    {
        ...
        Create([Bind("DateDepotAppareil,Appareil,Etat,SymptomesPanne,ClientId")]DemandeReparation demandeReparation)
        {
            if (ModelState.IsValid)
            {
                // Récupération des informations du client
                var client = await
                _clientRepository.GetClientByIdAsync(demandeReparation.ClientId);
                if (client == null)
                {
        ...
        [ValidateAntiForgeryToken]
        public async Task<IActionResult> Edit(int id,
        [Bind("Id,DateDepotAppareil,Appareil,Etat,SymptomesPanne,ClientId")]DemandeReparation demandeReparation)
        {
            ...
            {
                await
                _demandeReparationRepository.UpdateAsync(demandeReparation);
